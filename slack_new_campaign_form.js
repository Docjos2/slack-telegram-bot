...
let app; // Declare app variable outside the try block
let logger; // Declare logger variable outside

try {
    console.log("Attempting to initialize Bolt App..."); // Log before init

    // Initialize Bolt App
    // Ensure SLACK_BOT_TOKEN (xoxb-), SLACK_APP_TOKEN (xapp-), SLACK_SIGNING_SECRET, and N8N_WEBHOOK_URL are in your .env file / Railway Vars
    app = new App({ // Assign to the outer variable
    ...
    logger = app.logger; // Assign to the outer variable
    console.log("Bolt App initialized successfully."); // Log after successful init

} catch (initError) {
    // --- Catch errors during new App() initialization ---
    console.error("💥 CRITICAL ERROR DURING BOLT APP INITIALIZATION:", initError);
    // Log the error object structure if possible
    console.error("Initialization Error Details:", JSON.stringify(initError, null, 2));
    // Attempt to log specific properties if they exist
    if (initError.code) console.error("Error Code:", initError.code);
    if (initError.message) console.error("Error Message:", initError.message);
    // Exit immediately if initialization fails
    process.exit(1);
}
...
// Log the URL source on startup (only if logger was initialized)
if (logger) {
    if (N8N_WEBHOOK_URL_FROM_ENV) {
     ...
     console.warn("Logger not initialized, cannot log Webhook URL source.");
}
const N8N_WEBHOOK_URL = N8N_WEBHOOK_URL_FROM_ENV || N8N_WEBHOOK_URL_FALLBACK;


// --- Slash Command Handler (/new_campaign_v2) ---
// Check if app was initialized before defining listeners
if (app) {
    ...
    // --- Start the App ---
    (async () => {
      // Check if app was initialized before starting
      if (!app) {
          console.error("💥 Bolt App was not initialized. Cannot start.");
          process.exit(1);
      }
      try {
        const port = process.env.PORT || 3000;
        await app.start(port);
        const startupMessage = `⚡️ Bolt app (Simple Modal v4 - Catch Init Err) is running on port ${port} in Socket Mode!`; // Updated startup message
        console.log(startupMessage);
        // Ensure logger exists before using it
        if (logger) {
            logger.info(startupMessage);
        } else {
            console.warn("Logger not available at final startup log point.");
        }
      } catch (error) {
    ...
    // This block executes if app initialization failed in the first try/catch
    console.error("💥 Bolt App initialization failed. Exiting.");
    // process.exit(1); // Already exited in the catch block, but good for clarity
}
